	...
	<view class="duty-calendar">
		...
		<view class="date-grid">
			...
			<view v-for="item in currentMonthDays" :key="`current-${item.cDay}`" class="day-cell" :class="dayClass(item)" hover-class="day-cell--pressed" @click="selectDay(item)">
				...
				<text v-if="item.isOnDuty" class="day-tag tag-duty">值</text><text v-if="item.isVacation" class="day-tag tag-vacation">休</text><text v-if="item.isCompensatoryLeave" class="day-tag tag-workday">班</text>
			</view>
...
const props = defineProps({ nowDate: { type: String, default: () => new getDate().dateFormat() }, selectedDate: { type: String, default: '' }, shiftsNum: { type: Number, default: 0 }, latestDate: { type: Number, default: null }, holidayMap: { type: Object, default: () => ({}) } })
...
const getHolidayData = day => {
	...
	const lunarLabel = calendarHoliday?.displayText || (builtInHoliday ? `${day.cYear} ${builtInHoliday.name}（安排待更新）` : day.IDayCn)
...
const init = () => {
	...
	nowYear.value = date.getFullYear(); nowMonth.value = date.getMonth()
	nowPeasant.value = lunarCalendar.solar2lunar(nowYear.value, nowMonth.value, date.getDate())
	const daysInMonth = getDate.getDays({ year: nowYear.value, month: nowMonth.value })
	const firstWeekday = getDate.getMonthWeekBegin({ year: nowYear.value, month: nowMonth.value })
...
watch(() => [props.nowDate, props.shiftsNum, props.latestDate, props.holidayMap], init, { immediate: true })
</script>
